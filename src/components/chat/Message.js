...
import avatarUser from '../../assets/icon/user.png'
...
function Message(props) {
    let isAdmin = props.nameUser == "admin" ? true : false;
    return (
        <MessageStyle>
            <div className="message" style={{ alignItems: isAdmin ? "start" : "end" }}>
                <div className="info-message" style={{ flexDirection: isAdmin ? "row" : "row-reverse" }}>
                    <img src={isAdmin ? avatarAdmin : avatarUser} style={{ height: 25, width: 25, backgroundColor: 'white', borderRadius: 50 }}></img>
